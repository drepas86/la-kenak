...
$tdir = "uploadimg/images/topo/";   // Διαδρομή για την μικρότερη εικόνα του ΠΕΑ
...
$theight = "800";   // Μέγιστο ύψος εικόνας για ΠΕΑ - Ορίζω γύρω στα 800-1024 για να βγει η εικόνα γύρω στα 150kb
   ...
   // Φόρμα εικόνας   ?>
  <form method="post" action="kenak.php?page=2#tab-topoimg" enctype="multipart/form-data">
   Αρχείο τοπογραφίας:<br />
  <input type="file" name="imagefile" class="form">
  <br /><br />
  <input name="submittopo" type="submit" value="Ανέβασε" class="formtopo">  <input type="reset" value="Καθαρισμός" class="formtopo">
  </form>
<? 

	if (isset($_POST['submittopo'])) {   // Ανέβασμα / αλλαγή μεγέθους
  ...
  if ($_FILES['imagefile']['type'] == "image/jpg" || $_FILES['imagefile']['type'] == "image/jpeg" || $_FILES['imagefile']['type'] == "image/pjpeg") {
    ...
    if ($copy) {   // If The Script Was Able To Copy The Image To It's Permanent Location
      ...
      imagejpeg($dimg, "$tdir" . 'user-'.$_SESSION['user_id'].'meleti-'.$_SESSION['meleti_id'].'-topo.jpg');   // Αποθήκευση της εικόνας
	  $filesize = round(filesize($tdir.'user-'.$_SESSION['user_id'].'meleti-'.$_SESSION['meleti_id'].'-topo.jpg')/1024, 2);
...
if (file_exists($tdir.'user-'.$_SESSION['user_id'].'meleti-'.$_SESSION['meleti_id'].'-topo.jpg')) {
    echo '<br/>Προεπισκόπιση εικονας ('.round(filesize($tdir.'user-'.$_SESSION['user_id'].'meleti-'.$_SESSION['meleti_id'].'-topo.jpg')/1024, 2).'Kb):
	  <br/>';
	echo '<img src="'.$tdir.'user-'.$_SESSION['user_id'].'meleti-'.$_SESSION['meleti_id'].'-topo.jpg">';
} else {
    echo "Δεν έχετε ανεβάσει ακόμα εικόνα για το τοπογραφικό της μελέτης/ΠΕΑ";
}
 ?>