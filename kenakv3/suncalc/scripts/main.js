...
$(document).ready(function() {
	...
	(function init() {
		...
		$('#21jun09-link').click(function() {
			setDate(new Date("Jun 21, 2012 09:00:00"));
			updateResult();
			updateAddressHash();
			return false;
		});
		$('#21jun12-link').click(function() {
			setDate(new Date("Jun 21, 2012 12:00:00"));
			updateResult();
			updateAddressHash();
			return false;
		});
		$('#21jun15-link').click(function() {
			setDate(new Date("Jun 21, 2012 15:00:00"));
			updateResult();
			updateAddressHash();
			return false;
		});
		$('#21dec09-link').click(function() {
			setDate(new Date("Dec 21, 2012 09:00:00"));
			updateResult();
			updateAddressHash();
			return false;
		});
		$('#21dec12-link').click(function() {
			setDate(new Date("Dec 21, 2012 12:00:00"));
			updateResult();
			updateAddressHash();
			return false;
		});
		$('#21dec15-link').click(function() {
			setDate(new Date("Dec 21, 2012 15:00:00"));
			updateResult();
			updateAddressHash();
			return false;
		});
		...
		if ($.address.pathNames().length != 3) {
			...
			if (lastView) {
				...
				setLocation(new google.maps.LatLng(38.028622, 23.75473), "Athens", new google.maps.LatLng(45, -10), 2);
			}
			setDate(date);
			updateResult();
			updateAddressHash();
		...
		$('#more-detailed-link').click(function() {
			...
			$(this).html((moreDetailed ? "Λιγότερες" : "Περισσότερες") + " πληροφορίες &raquo;");
