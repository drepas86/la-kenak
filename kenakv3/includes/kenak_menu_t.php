...
?>
					...
					<?php
					$strSQL = "SELECT * FROM kataskeyi_t_" . $it[$j] . " WHERE user_id=".$_SESSION['user_id']." AND meleti_id=".$_SESSION['meleti_id'];
					...
					$i++;
					$tid=$objResult["id"];
					$strSQL1 = "SELECT * FROM kataskeyi_an_" . $it[$j]." WHERE id_toixoy=".$tid. " AND user_id=".$_SESSION['user_id']." AND meleti_id=".$_SESSION['meleti_id'];
...
<!--        Κρυφό div για βοήθεια μήκος των υποστηλωμάτων                           -->
		...
		Η οριζόντια διάσταση των υποστυλωμάτων. Η κατακόρυφη διάσταση αυτών είναι το ύψος του τοίχου μείον το ύψος της δοκού.<br/>
		<hr>
		Συμβατικός τρόπος υπολογισμού του εμβαδού που καταλαμβάνει ο φέρων οργανισμός του κτηρίου ως ποσοστό επί της επιφάνειας της όψης του
		σε περίπτωση που δεν είναι εφικτή η αποτύπωση του φέροντος οργανισμού.<br/>
		<table border="1">
		<tr>
		<td>Έτος έκδοσης οικ. αδ.</td><td>Τύπος κτιρίου</td><td>Όροφοι <=5</td><td>Όροφοι >5</td>
		</tr>
		<tr>
		<td>Προ του 1981</td><td>Γωνιακό</td><td>15%</td><td>22%</td>
		</tr>
		<tr>
		<td>Προ του 1981</td><td>Μη γωνιακό</td><td>25%</td><td>30%</td>
		</tr>
		<tr>
		<td>1981 έως 1999</td><td>Γωνιακό</td><td>18%</td><td>25%</td>
		</tr>
		<tr>
...
<script language="JavaScript">
document.getElementById("t_thermo1").selectedIndex="13";
document.getElementById("t_thermo2").selectedIndex="13";
document.getElementById("t_thermo3").selectedIndex="13";
document.getElementById("t_thermo4").selectedIndex="13";
document.getElementById("t_thermodap1").selectedIndex="13";
