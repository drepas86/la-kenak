...
if(add_new_table("domika_anoigmata")){
	...
	//Αν λείπει η γραμμή προσθήκη πριν το update
	$strSQL = "SELECT * FROM kataskeyi_an_s WHERE user_id=".$_SESSION['user_id']." AND meleti_id=".$_SESSION['meleti_id'];
	$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
	$num_rows = mysql_num_rows($objQuery);
	if ($num_rows == 0){
	$strSQL = "INSERT INTO kataskeyi_an_s ";
	$strSQL .= "(user_id, meleti_id, plaisio, ukoyf, platos, pane, upane) ";
	$strSQL .= "VALUES (".$_SESSION['user_id'].", ".$_SESSION['meleti_id'].", 'πλαίσιο', 1, 1, 'τύπος', 1)";
	$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
	}
