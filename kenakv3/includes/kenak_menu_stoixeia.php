...
?>
			...
			<div id="tabvanilla" class="widget" style="display:none;">
					...
					<li><a href="#tab-img">Εικόνα ΠΕΑ</a></li>
					<li><a href="#tab-topoimg">Εικόνα Τοπογραφικό</a></li>
					</ul>  
					
					<div id="tab-meletis" class="tabdiv"> 
					
					<h3>Εύρεση στο χάρτη</h3>
					<?php include("includes/gmap1.php"); ?>
					...
					$strSQL = "SELECT * FROM kataskeyi_meletis WHERE user_id=".$_SESSION['user_id']." AND meleti_id=".$_SESSION['meleti_id'];
					$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
					$num_rows = mysql_num_rows($objQuery);
					if ($num_rows == 0){
					$strSQL = "INSERT INTO kataskeyi_meletis ";
					$strSQL .= "(user_id, meleti_id, project, place, owner, engs, owner_status, address, contact_type , contact_name, contact_tel, contact_mail) ";
					$strSQL .= "VALUES (".$_SESSION['user_id'].", ".$_SESSION['meleti_id'].", 'νεόκτιστη οικοδομή', 'Περιοχή μελέτης', 'Όνομα ιδιοκτήτη', 'Μελετητής', 1, 'Διεύθυνση 1', 3, 'Ονοματεπώνυμο υπευθύνου', 1234567890, '@ υπευθύνου');";
					$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
					}
					
					
					
					$kataskeyi_meletis_array = get_times_all_user("*", "kataskeyi_meletis", $_SESSION['user_id'], $_SESSION['meleti_id']);
...
<!--					<td><div align="center"><?=$kataskeyi_meletis_address;?></div></td>-->
					<td><input type="text" id="<?=$vasi."_address"?>" name="<?=$vasi."_address"?>" value="<?=$kataskeyi_meletis_address;?>" maxlength="200" size="35" /></td>
					</tr>
					<tr>
...
<!--					<td><?=$kataskeyi_meletis_contact_mail;?></td>-->
					...
					<?php
					//Δήλωση εαν δεν υπάρχει η γραμμή.
					$strSQL = "SELECT * FROM kataskeyi_meletis_topo WHERE user_id=".$_SESSION['user_id']." AND meleti_id=".$_SESSION['meleti_id'];
					$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
					$num_rows = mysql_num_rows($objQuery);
					if ($num_rows == 0){
					$strSQL = "INSERT INTO kataskeyi_meletis_topo ";
					$strSQL .= "(user_id, meleti_id, sxedio, odos, apostaseis, klisi, prosb, pol_grafeio, pol_year, pol_number, pol_year_complete, pol_type) ";
					$strSQL .= "VALUES (".$_SESSION['user_id'].", ".$_SESSION['meleti_id'].", 'εντός', 'αγροτική', 0, 0, 0, 'Όνομα πολεοδομικού γραφείου', 1234, 1234567890, 1234, 1);";
					$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
					}
...
<!--					<td><?=$kataskeyi_meletistopo_pol_type;?></td> -->
					...
					$num_rows = mysql_num_rows($objQuery);
					if ($num_rows == 0){
					$strSQL = "INSERT INTO kataskeyi_stoixeia ";
					$strSQL .= "(user_id, meleti_id, zwni, climate_data, xrisi, nero_dikt, velt_klisi, iliakos, anigmeni_thermo, aytomatismoi, kaminades, eksaerismos, anem_orofis) ";
					$strSQL .= "VALUES (".$_SESSION['user_id'].", ".$_SESSION['meleti_id'].", 'a', 1, 1, 1, 1, 0, 165, 3, 0, 0, 0);";
					$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
					}
...
<!--					<td><?=$kataskeyi_meletistopo_klisi;?> deg</td>-->
					...
					<div id="tab-img" class="tabdiv">
					<?php include("uploadimg/upload.php"); ?>
					</div><!--/ΕΙΚΟΝΑ-->
					<div id="tab-topoimg" class="tabdiv">
					<?php include("uploadimg/uploadtopoimg.php"); ?>
					</div><!--/Τοπογραφία ΕΙΚΟΝΑ-->
