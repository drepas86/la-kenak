...
*/
	...
	if (isset($_POST['submit4'])) { // υποβλήθηκε η φόρμα.
		...
		$required_fields = array('name', 'platos_toixoy', 'apost_ar', 'mikos_ar', 'apost_de', 'mikos_de', 'pros');
		$errors = array_merge($errors, check_required_fields($required_fields, $_POST));

		$fields_with_lengths = array('name' => 30, 'platos_toixoy' => 30, 'apost_ar' => 30, 'mikos_ar' => 30, 'apost_de' => 30, 'mikos_de' => 30, 'pros' => 30);
		$errors = array_merge($errors, check_max_field_lengths($fields_with_lengths, $_POST));

		$name = trim(mysql_prep($_POST['name']));
		$platos_toixoy = trim(mysql_prep($_POST['platos_toixoy']));
		$apost_ar = trim(mysql_prep($_POST['apost_ar']));
		$mikos_ar = trim(mysql_prep($_POST['mikos_ar']));
		$apost_de = trim(mysql_prep($_POST['apost_de']));
		$mikos_de = trim(mysql_prep($_POST['mikos_de']));
		...
		if ( empty($errors) ) {
			...
			"?platos_toixoy=" . $platos_toixoy . "&apost_ar=" . $apost_ar . "&mikos_ar=" . 
			$mikos_ar . "&apost_de=" . $apost_de . "&mikos_de=" . $mikos_de . "&apostemp=" . 
			$apost_empod . "&ypsosemp=" . $ypsos_empod . "&pros=" . $pros . "&name=" . $name . "\"></img>";
			
			
			echo "<img src=\"images/pleyrika.jpg\"></img><br/>";
			...
			"<td>Μήκος εμποδίου αριστερά</td><td>Απόσταση εμποδίου δεξιά</td><td>Μήκος εμποδίου δεξιά</td><td>Προσανατολισμός</td></tr><tr><td>" . 
			$name . "</td><td>" . $platos_toixoy . "</td><td>" . $apost_ar . "</td><td>" . $mikos_ar . "</td><td>" . 
			$apost_de . "</td><td>" . $mikos_de . "</td><td>" . $pros . "</td></tr></table><br/><br/>";
			
			//Βρες τις μοίρες για τις γωνίες και στρογγυλοποίησέ τες χωρίς δεκαδικά
			$tantoixoyar = ($mikos_ar / ($apost_ar + ($platos_toixoy/2)));
			$degtoixar = rad2deg(atan($tantoixoyar));
			$degtoixoyar = round($degtoixar);
			
			$tantoixoyde = ($mikos_de / ($apost_de + ($platos_toixoy/2)));
			...
			"<td>Προσανατολισμός επιφάνειας</td><td>f_fin_h Τοίχου αριστερά</td><td>f_fin_c Τοίχου αριστερά</td><td>f_fin_h τοίχου δεξιά</td><td>f_fin_c τοίχου δεξιά</td><td>f_fin_h Συνολικό</td><td>f_fin_c Συνολικό</td></tr><tr><td>" . 
			$name . "</td><td>" . $degtoixoyar . " μοίρες</td><td>" . $degtoixoyde . " μοίρες</td><td>Πίνακας: " . $pinakasthesisar . $pinakasthesisde . "</td><td>" . 
			$prosanatolismos_epif . "</td><td>" . $f_har . "</td><td>" . $f_car . "</td><td>" . $f_hde . "</td><td>" . $f_cde . "</td><td>" . $f_h_syn . "</td><td>" . $f_c_syn . "</td></tr></table>";
