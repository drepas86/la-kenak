...
<?php
$it[1]="b";
$dt[1]="Διαγραφή Σκίασης Βόρειου τοίχου";
$pt[1]="Προσθήκη Σκίασης Βόρειου τοίχου";
$it[2]="a";
$dt[2]="Διαγραφή Σκίασης Ανατολικού τοίχου";
$pt[2]="Προσθήκη Σκίασης Ανατολικού τοίχου";
$it[3]="n";
$dt[3]="Διαγραφή Σκίασης Νότιου τοίχου";
$pt[3]="Προσθήκη Σκίασης Νότιου τοίχου";
$it[4]="d";
$dt[4]="Διαγραφή Σκίασης Δυτικού τοίχου";
$pt[4]="Προσθήκη Σκίασης Δυτικού τοίχου";

for ($j=1;$j<=4;$j++){
?>
			<div id="sk_toix_<?=$it[$j];?>" class="tabdiv"> 
					<form name="frmMain<?=$j;?>" id="frmMain<?=$j;?>" action="kenak.php" method="post" OnSubmit="return onDelete();">
					<?php
					$strSQL = "SELECT * FROM kataskeyi_skiaseis_t_".$it[$j]. " WHERE user_id=".$_SESSION['user_id']." AND meleti_id=".$_SESSION['meleti_id'];
					...
					{
					$i++;
					$name_toixoy_array = get_times_user("name", "kataskeyi_t_".$it[$j], $objResult["id_toixoy"], $_SESSION['user_id'], $_SESSION['meleti_id']);
	...
	<!--				<td><div align="center"><?=$objResult["id"];?></div></td> -->
					...
					<input type="submit" name="delete_sk_t_<?=$it[$j];?>" value="<?=$dt[$j];?>">
					...
					<?php
						echo "<tr>";
						$id_toixoy = dropdown1("SELECT id, name FROM kataskeyi_t_".$it[$j]. " WHERE user_id=".$_SESSION['user_id']." AND meleti_id=".$_SESSION['meleti_id'], "id", "name");
						...
						echo "<input type=\"submit\" name=\"" . $vasi . "_sk_t_" . $it[$j] . "\" value=\"".$pt[$j]."\" />";
					?>
					</form>
			</div>
...
<!--        Κρυφό div για βοήθεια                                                   -->
		...
		<b>Σκιάσεις Τοίχων</b><hr>
		Στη σελίδα αυτή γίνεται η εισαγωγή των σκιάσεων των τοίχων. 
...
x=x+"|"+m1;
x="./includes/save_sk_t.php?rec="+x;
